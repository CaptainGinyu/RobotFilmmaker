    ...
    while True:
        ...
        if (flag % 6 == 0):
            ...
            for (x, y, w, h) in faces:
                ...
                width_face = y +h;
                try:
                    dist = str(float(arduinoSerial.readline().strip().decode()))
                    g.write(dist)
                    print(dist)
                    f.write(str(length_face*width_face))
                    f.write("\n")
                    g.write("\n")  
                except:
                    pass
                ...
                if (flag % 30 == 0):
                    ...
                    length_face = x + w;
                    width_face = y +h;
                    # thetha = round((math.acos((math.pow(dist1,2) + math.pow(dist2,2) - math.pow(pow(desired_center[0] - center[0], 2)+ math.pow(desired_center[1] - center[1], 2), 0.5)) / (2* dist1 * dist2)))* 180 / math.pi) 
                    # # ensure there is significant movement in the
                    # if numpy.abs(movement[0]) > 40 and numpy.abs(movement[1]) > 40:
                    #     if movement[0] > 0:
                    #         dirX = "East"
