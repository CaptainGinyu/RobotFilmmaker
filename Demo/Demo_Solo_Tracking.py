...
TIMER_FORWARD = TIMER_FACE  # Robot movement forward and backwards every x seconds
TIMER_SERVO = 1.0  # Servo movement every x seconds
TIMER_FIREBASE = 2  # Firebase messaging every x seconds

CART_FORW_S = 0.40  # Move cart forward if face takes up less than x of the frame
CART_BACK_S = 0.67  # Move cart backward if face takes up more than x of the frame

MOVE_DIFF_X_UNIT = 45  # Servo moves if x or y-axis difference btwen center of face and frame is greater than this (45, 40 is good number too)
MOVE_DIFF_Y_UNIT = 40
UNIT_DEG_X = 15  # Each UNIT_DEG_X or UNIT_DEG_Y x or y-axis difference, the robot will move 1 degrees
UNIT_DEG_Y = 15

MOVE_DIFF_X_BIG = 45  # Servo moves if x or y-axis difference between center of face and frame is greater than this (35, 30 default)
MOVE_DIFF_Y_BIG = 40
MOVE_DIFF_X_SMALL = MOVE_DIFF_X_BIG / 2  # Servo moves if x or y-axis difference is greater than this smaller amount
MOVE_DIFF_Y_SMALL = MOVE_DIFF_Y_BIG / 2
...
while 1:
    ...
    # 3. If forward timer is up and face detected, move the car forward/backward
    if forward_tdiff > TIMER_FORWARD and target == 1:
        ...
        if numpy.abs(length_face) < height * CART_FORW_S or numpy.abs(width_face) < width * CART_FORW_S:
            # arduinoSerial.write(bytes('f'))
            print ('f')
        if numpy.abs(length_face) > height * CART_BACK_S or numpy.abs(width_face) > width * CART_BACK_S:
            # arduinoSerial.write(bytes('b'))
            print ('b')

    # 4. If servo timer is up and face detected, move the servo
        ...
        # Check is there is a large movement
        '''
        if numpy.abs(movement[0]) > MOVE_DIFF_X_BIG or numpy.abs(movement[1]) > MOVE_DIFF_Y_BIG:
            if movement[0] > 0:
                dirX = "East"
                #arduinoSerial.write(bytes('rrr'))
                print ('rrr')
            else:
                dirX = "West"
                #arduinoSerial.write(bytes('lll'))
                print ('lll')
            if movement[1] > 0:
                dirY = "North"
                #arduinoSerial.write(bytes('uuu'))
                print ('uuu')
            else:
                dirY = "South"
                #arduinoSerial.write(bytes('ddd'))
                print ('ddd')

        # If no large movement, check if there is a smaller movement
        if numpy.abs(movement[0]) > MOVE_DIFF_X_SMALL or numpy.abs(movement[1]) > MOVE_DIFF_Y_SMALL:
            if movement[0] > 0:
                dirX = "East"
