...
def training():
    ...
    while True:
        ...
        mini = cv2.resize(im,
                          (int(im.shape[1] / size), int(im.shape[0] / size)))  # Resize the image to speed up detection
    ...
    if UPLOAD_KEVIN:
        ...
        for root, dirs, files in os.walk(folder_target):
            for myfile in files:
                s3.meta.client.upload_file(os.path.join(root, myfile), AWS_BUCKET_UPLOAD,
                                           os.path.join('Kevin/', myfile))
